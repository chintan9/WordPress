...
var imageEdit = window.imageEdit = {
  ...
  handleCropToolClick : function(postid, nonce, cropButton) {
    ...
    if (0 === selection.x1 && 0 === selection.y1 && 0 === selection.x2 &&
        0 === selection.y2) {
  ...
  intval : function(f) {
    /*
     * Bitwise OR operator: one of the obscure ways to truncate floating point
     * figures, worth reminding JavaScript doesn't have a distinct "integer"
     * type.
     */
    return f | 0;
  },
  ...
  /**
   * Adds the disabled attribute and class to a single form element or a field
   * set.
   *
   * @since 2.9.0
   *
  ...
  setDisabled : function(el, s) {
    /*
     * `el` can be a single form element or a fieldset. Before #28864, the
     * disabled state on some text fields  was handled targeting $('input', el).
     * Now we need to handle the disabled state on buttons too so we can just
     * target `el` regardless if it's a single element or a fieldset because
     * when a fieldset is disabled, its descendants are disabled too.
     */
    if (s) {
   ...
   * @param {number} postid The post id.
   *
   * @return {string} The value from the imagedit-save-target input field when
   *     available,
   *                  or 'full' when not available.
   */
  getTarget : function(postid) {
    return $('input[name="imgedit-target-' + postid + '"]:checked',
   ...
   *
   * @param {number} postid  The post id.
   * @param {number} setSize 0 or 1, when 1 the image resets to its original
   *     size.
   *
   * @return {string} JSON string containing the history or an empty string if
   *     no history exists.
   */
  ...
  refreshEditor : function(postid, nonce, callback) {
    ...
    img =
        $('<img id="image-preview-' + postid + '" alt="" />')
            .on('load', {history : data.history},
                function(event) {
                  var max1, max2, parent = $('#imgedit-crop-' + postid),
                                  t = imageEdit, historyObj;
                  ...
                  if ('' !== event.data.history) {
                    ...
                    if (historyObj[historyObj.length - 1].hasOwnProperty('c')) {
                      /*
                       * A crop action has completed and the crop button gets
                       * disabled ensure the undo button is enabled.
                       */
                  ...
                  if ($('#imgedit-history-' + postid).val() &&
                      $('#imgedit-undone-' + postid).val() === '0') {
                    $('input.imgedit-submit-btn', '#imgedit-panel-' + postid)
                        .removeAttr('disabled');
                  } else {
                    $('input.imgedit-submit-btn', '#imgedit-panel-' + postid)
                        .prop('disabled', true);
            ...
            .on('error',
                function() {
                  $('#imgedit-crop-' + postid)
                      .empty()
                      .append('<div class="error"><p>' + imageEditL10n.error +
                              '</p></div>');
                  t.toggleEditor(postid, 0);
                })
  ...
  action : function(postid, nonce, action) {
    ...
    if ('scale' === action) {
      ...
      h = $('#imgedit-scale-height-' + postid), fw = t.intval(w.val()),
      fh = t.intval(h.val());
   ...
   * @param {string}  nonce    The nonce to verify the request.
   *
   * @return {boolean|void}  If the actions are successfully saved a response
   *     message is shown.
   *                         Returns false if there is no image editing history,
   *                         thus there are not edit-actions performed on the
   * image.
   */
  save : function(postid, nonce) {
    var data, target = this.getTarget(postid),
              history = this.filterHistory(postid, 0), self = this;
    ...
    $.post(ajaxurl, data, function(r) {
      ...
      if (ret.error) {
        $('#imgedit-response-' + postid)
            .html('<div class="error"><p>' + ret.error + '</p></div>');
      ...
      if (ret.thumbnail) {
        $('.thumbnail', '#thumbnail-head-' + postid)
            .attr('src', '' + ret.thumbnail);
      }

      if (ret.msg) {
        $('#imgedit-response-' + postid)
            .html('<div class="updated"><p>' + ret.msg + '</p></div>');
      }
   ...
   *
   * @return {void|promise} Either returns void if the button was already
   *     activated
   *                        or returns an instance of the image editor, wrapped
   * in a promise.
   */
  open : function(postid, nonce, view) {
    ...
    var dfd, data,
        elem = $('#image-editor-' + postid), head = $('#media-head-' + postid),
        btn = $('#imgedit-open-btn-' + postid), spin = btn.siblings('.spinner');

    /*
     * Instead of disabling the button, which causes a focus loss and makes
     * screen readers announce "unavailable", return if the button was already
     * clicked.
     */
    ...
    dfd = $.ajax({
             ...
             type : 'post',
             data : data,
             beforeSend : function() { btn.addClass('button-activated'); }
           }).done(function(html) {
      elem.html(html);
  ...
  initCrop : function(postid, image, parent) {
    var t = this, selW = $('#imgedit-sel-width-' + postid),
        selH = $('#imgedit-sel-height-' + postid), $image = $(image), $img;
    ...
    t.iasapi = $image.imgAreaSelect({
      ...
      onInit : function(img) {
        ...
        parent.children().on('mousedown, touchstart', function(e) {
          ...
          if (e.shiftKey) {
            ...
            ratio = (sel && sel.width && sel.height)
                        ? sel.width + ':' + sel.height
                        : defRatio;
          }

          t.iasapi.setOptions({aspectRatio : ratio});
        });
      },
       ...
       * @return {object}
       */
      onSelectEnd : function(img, c) { imageEdit.setCropSelection(postid, c); },
  ...
  addStep : function(op, postid, nonce) {
    var t = this, elem = $('#imgedit-history-' + postid),
        ...
        undone = $('#imgedit-undone-' + postid), pop = t.intval(undone.val());
  ...
  undo : function(postid, nonce) {
    var t = this, button = $('#image-undo-' + postid),
        elem = $('#imgedit-undone-' + postid), pop = t.intval(elem.val()) + 1;
    ...
    t.refreshEditor(postid, nonce, function() {
      ...
      // When undo gets disabled, move focus to the redo button to avoid a focus
      // loss.
  ...
  redo : function(postid, nonce) {
    var t = this, button = $('#image-redo-' + postid),
        elem = $('#imgedit-undone-' + postid), pop = t.intval(elem.val()) - 1;
    ...
    t.refreshEditor(postid, nonce, function() {
      ...
      // When redo gets disabled, move focus to the undo button to avoid a focus
      // loss.
      if (0 === pop) {
        $('#image-undo-' + postid).focus();
      }
   ...
   *                        void when the value is not numeric or when the
   * operation is successful.
   */
  setNumSelection : function(postid, el) {
    var sel, elX = $('#imgedit-sel-width-' + postid),
             elY = $('#imgedit-sel-height-' + postid),
             x = this.intval(elX.val()), y = this.intval(elY.val()),
             img = $('#image-preview-' + postid), imgh = img.height(),
             imgw = img.width(), sizer = this.hold.sizer, x1, y1, x2, y2,
             ias = this.iasapi;
  ...
  setRatioSelection : function(postid, n, el) {
    ...
    if (x && y) {
      this.iasapi.setOptions({aspectRatio : x + ':' + y});
