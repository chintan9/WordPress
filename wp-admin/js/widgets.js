...
window.wpWidgets = {
  ...
  init : function() {
    var rem, the_id, self = this, chooser = $('.widgets-chooser'),
    ...
    $(window).on('beforeunload.widgets', function(event) {
      ...
      if (0 !== dirtyWidgetIds.length) {
        unsavedWidgetsElements =
            $('#widgets-right').find('.widget').filter(function() {
              return -1 !== dirtyWidgetIds.indexOf(
                                $(this).prop('id').replace(/^widget-\d+_/, ''));
            });
        ...
        // Bring the first unsaved widget into view and focus on the first
        // tabbable field.
    ...
    $(document.body).bind('click.widgets-toggle', function(e) {
      var target = $(e.target), css = {}, widget, inside, targetWidth,
          widgetWidth, margin, saveButton, widgetId,
          toggleBtn = target.closest('.widget').find(
      ...
      if (target.parents('.widget-top').length &&
        ...
        // Save button is initially disabled, but is enabled when a field is
        // changed.
        ...
        if (inside.is(':hidden')) {
          ...
          inside.slideDown('fast', function() { widget.addClass('open'); });
        } else {
          /*
           * Don't change the order of attributes changes and animation:
           * it's important for screen readers, see ticket #31476.
    ...
    /**
     * Opens and closes previously closed Sidebars when Widgets are dragged
     * over/out of them.
     */
    sidebars.droppable({
      tolerance : 'intersect',
      ...
      over : function(event) {
        ...
        if (wpWidgets.hoveredSidebar && !$wrap.is(wpWidgets.hoveredSidebar)) {
          // Close the previous Sidebar as the Widget has been dragged onto
          // another Sidebar.
    ...
    sidebars
        .sortable({
          ...
          refreshPositions : true,
          start : function(event, ui) {
            var height, $this = $(this), $wrap = $this.parent(),
                        inside = ui.item.children('.widget-inside');

            if (inside.css('display') === 'block') {
              ui.item.removeClass('open');
              ui.item.find('.widget-top button.widget-action')
            ...
            if (!$wrap.hasClass('closed')) {
              ...
              // Prevents jumping when dragging a widget from an open sidebar to
              // a closed sidebar below.
              height = ui.item.hasClass('ui-draggable') ? $this.height()
                                                        : 1 + $this.height();
          ...
          activate : function() { $(this).parent().addClass('widget-hover'); },
          ...
          receive : function(event, ui) {
            ...
            // If the last widget was moved out of an orphaned sidebar, close
            // and remove it.
            if ($sender.attr('id').indexOf('orphaned_widgets') > -1 &&
                !$sender.children('.widget').length) {
              $sender.parents('.orphan-sidebar')
    ...
    $('#available-widgets').droppable({
      tolerance : 'pointer',
      accept : function(
          o) { return $(o).parent().attr('id') !== 'widget-list'; },
      drop : function(e, ui) {
    ...
    $('#widgets-right .widgets-holder-wrap').each(function(index, element) {
      var $element = $(element),
          ...
          id = $element.find('.widgets-sortables').attr('id'), li = $('<li>'),
          button = $('<button>', {
                     type : 'button',
                     'aria-pressed' : 'false',
    ...
    $('#available-widgets .widget .widget-top')
        .on('click.widgets-chooser', function() {
          ...
          if ($widget.hasClass('widget-in-question') ||
            ...
            // Add CSS class and insert the chooser after the widget
            // description.
            ...
            chooser.slideDown(300, function() {
              // Update the toggle button aria-expanded attribute after previous
              // DOM manipulations.
  ...
  saveOrder : function(sidebarId) {
    ...
    $.post(ajaxurl, data, function() {
      $('#inactive-widgets-control-remove')
          .prop('disabled', !$('#wp_inactive_widgets .widget').length);
  ...
  save : function(widget, del, animate, order) {
    var self = this, data, a,
        ...
        form = widget.find('form'), isAdd = widget.find('input.add_new').val();

    if (!del && !isAdd && form.prop('checkValidity') &&
    ...
    $.post(ajaxurl, data, function(r) {
      ...
      if (del) {
        ...
        if (r && r.length > 2) {
          ...
          widget.find('.widget-control-save')
              .prop('disabled', true)
  ...
  addWidget : function(chooser) {
    ...
    if ('multi' === add) {
      widget.html(widget.html().replace(
          /<[^<>]+>/g, function(m) { return m.replace(/__i__|%i%/g, n); }));
    ...
    /*
     * Check if any part of the sidebar is visible in the viewport. If it is,
     * don't scroll. Otherwise, scroll up to so the sidebar is in view.
     *
     * We do this by comparing the top and bottom, of the sidebar so see if they
     * are within the bounds of the viewport.
     */
    ...
    if (viewportTop > sidebarBounds.bottom ||
        viewportBottom < sidebarBounds.top) {
      $('html, body').animate({scrollTop : sidebarBounds.top - 130}, 200);
    }
  ...
  closeChooser : function() {
    ...
    $('.widgets-chooser').slideUp(200, function() {
      ...
      widgetInQuestion.find('.widget-action')
          .attr('aria-expanded', 'false')
          .focus();
    });
  },
  ...
  /**
   * Closes a Sidebar that was previously closed, but opened by dragging a
   * Widget over it.
   *
